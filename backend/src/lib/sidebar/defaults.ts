// Default OPS sidebar layout. The shape mirrors the SidebarItem TS interface
// on the frontend. Backend stores this verbatim as JSON in `sidebar_config.layout`.
//
// SidebarItem has two flavors:
//   1. "standalone"  → top-level link, no children (e.g. Dashboard, Knowledge Base)
//   2. "section"     → top-level group with `children` (e.g. Sales > Pipeline + Leads)
//
// `route` is locked (used for routing). Editable fields: label, icon, color, visible, order.
